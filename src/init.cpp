...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/fifacoin-project/fifacoin>";
    const std::string URL_WEBSITE = "<https://fifacoin.org>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if fifacoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if fifacoin is started while in a temporary directory.\n",
